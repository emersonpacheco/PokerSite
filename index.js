...
function vencedor(){
  ...
  const valueCounts = countValues(hand);
  const valueCounts2 = countValues(hand2);
  if(handValue===10 && hand2Value===10){
    const values = hand.map(card => valueMap[card[0]]);
    const highestValue = Math.max(...values);
    const values2 = hand.map(card => valueMap[card[0]]);
    const highestValue2 = Math.max(...values2);
    if(highestValue>highestValue2){
      handValue=handValue-1;
    }else if(highestValue<highestValue2){
      handValue=handValue+1;
    }
  }if(handValue===9 && hand2Value===9){
    const pairs = Object.keys(valueCounts).filter(value => valueCounts[value] === 2);
    const highestPair = Math.max(...pairs.map(Number));
    const pairs2 = Object.keys(valueCounts).filter(value => valueCounts2[value] === 2);
    const highestPair2 = Math.max(...pairs2.map(Number));
    if(highestPair>highestPair2){
      handValue=handValue-1;
    }else if(highestPair<highestPair2){
      handValue=handValue+1;
    }
  }if(handValue===8 && hand2Value===8){
    const twoPair = Object.keys(valueCounts).filter(value => valueCounts[value] === 2);
    const highestPair = Math.max(...twoPair.map(Number));
    const twoPair2 = Object.keys(valueCounts).filter(value => valueCounts2[value] === 2);
    const highestPair2 = Math.max(...twoPair2.map(Number));
    if(highestPair>highestPair2){
      handValue=handValue-1;
    }else if(highestPair<highestPair2){
      handValue=handValue+1;
    }
  }if(handValue===7 && hand2Value===7){
    const Three = Object.keys(valueCounts).filter(value => valueCounts[value] === 3);
    const highestThree = Math.max(...Three.map(Number));
    const Three2 = Object.keys(valueCounts).filter(value => valueCounts2[value] === 3);
    const highestThree2 = Math.max(...Three2.map(Number));
    if(highestThree>highestThree2){
      handValue=handValue-1;
    }else if(highestThree<highestThree2){
      handValue=handValue+1;
    }
  }
}

function resultadoBalanço(){
